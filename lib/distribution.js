...
const runJob = async function (jobUri) {
  ...
  if (job.entity === 'agenda') {
    ...
    console.log('No method available for running distributions of entity type' + job.entity);
  ...
  return Promise.all(clearings).then(() => {
    ...
    for (var i = 0; i < pastMeetings.length; i++) {
      ...
      try {
        ...
        console.log(`Something went wrong while distributing past meeting ${meeting.uri}, skipping to the next one ...`);
      }
    }
  }).then(() => {
    console.log(`
